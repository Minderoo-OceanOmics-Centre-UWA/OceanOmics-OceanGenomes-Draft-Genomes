        ...
        for line in f:
            ...
            if parts[2] == "gene" and "Name" in attributes:
                ...
                if gene_name not in seen:
                    seen.add(gene_name)
                    found.append(gene_name)
                    if gene_name in gene_lengths:
                        gene_lengths[gene_name] = str(abs(end - start) + 1)

    missing = [g for g in REF_GENES if g not in found]
    extra = [g for g in found if g not in REF_GENES]
    order_ok = found == [g for g in REF_GENES if g in found]
...
def process_protein_lengths(prot_dir):
    annotation_name = get_annotation_name(gff_path)
    missing = []
...
def main(gff_path):
    ...
    gff_summary = process_gff(gff_path)
    prot_lengths = process_protein_lengths(prot_dir)
