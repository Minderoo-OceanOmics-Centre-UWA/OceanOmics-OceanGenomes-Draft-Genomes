process MEGAHIT {
    ...
    """
    # Always use the same output directory for this sample so MEGAHIT can resume.
    # Three cases: previously completed, partially completed, or fresh.
    if [ -s "${output_dir}/${prefix}.contigs.fa" ] || [ -f "${output_dir}/done" ]; then
        echo "Found completed MEGAHIT assembly, skipping..."
    elif [ -d "${output_dir}" ] && [ -f "${output_dir}/checkpoints.txt" ]; then
        ...
        if ! megahit \\
                ...
                --continue \\
                -o ${output_dir}; then
            echo "Resume failed, wiping checkpoint and restarting fresh..."
            rm -rf ${output_dir}
