...
workflow MITOGENOME_ANNOTATION {
    ...
    // Commented this out because the lca was erroring when the blast file was empty but it seems this was
    // actually caused by another processes. So i think it should work fine and then the species validation might work and lca push.
    // valid_blast_results = BLAST_BLASTN.out.filtered
    // .filter { meta, blast_file, gene_type, annotation_name ->
    //     def is_valid = blast_file.exists() && blast_file.size() > 0
    //     if (!is_valid) {
    //         log.warn "Skipping ${meta.id} ${gene_type}: file is missing or empty"
    //     }
    //     return is_valid
    // }
